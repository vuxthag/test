...
def analyze_video(video_file, models, reference, manual_view_choice="🤖 Tự Động Nhận Diện"):
    ...
    try:
        ...
        with st.spinner("🤖 AI đang phân tích chi tiết và tạo khuyến nghị cá nhân hóa..."):
            try:
                ai_recommendations = generate_ai_recommendations(analysis_data)
                analysis_data['recommendations'] = ai_recommendations
            except Exception as e:
                st.warning(f"⚠️ AI recommendations gặp lỗi: {str(e)}")
                from ai_recommendations import get_fallback_recommendations
                analysis_data['recommendations'] = get_fallback_recommendations(analysis_data)
...
with st.sidebar:
    ...
    """, unsafe_allow_html=True)
    from chatbot_assistant import render_chatbot
    with st.sidebar:
        render_chatbot()
